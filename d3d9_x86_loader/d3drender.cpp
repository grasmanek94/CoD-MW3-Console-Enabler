...
HRESULT CD3DFont::Print ( float x, float y, DWORD color, const char *szText, bool textcoloring )
{
	...
	DWORD RenderStateClipping = 0;
	DWORD RenderStateZEnable = 0;
	DWORD	fvf = 0;
	...
	m_pD3Ddev->GetRenderState(D3DRS_CLIPPING, &RenderStateClipping);
	m_pD3Ddev->GetRenderState(D3DRS_CLIPPING, &RenderStateZEnable);
	m_pD3Ddev->GetFVF(&fvf);
	m_pD3Ddev->GetTexture(0, &Texture);
	m_pD3Ddev->GetStreamSource(0, &VertexBuffer, &OffsetInBytes, &pStride);
	...
	//m_pD3Ddev->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_MODULATE );
	m_pD3Ddev->SetFVF(fvf);
	m_pD3Ddev->SetRenderState(D3DRS_CLIPPING, RenderStateClipping);
	m_pD3Ddev->SetRenderState(D3DRS_ZENABLE, RenderStateZEnable);
	m_pD3Ddev->SetTexture(0, Texture);
	m_pD3Ddev->SetStreamSource(0, VertexBuffer, OffsetInBytes, pStride);
	//
	this->EndRender();
...
HRESULT CD3DRender::Initialize ( IDirect3DDevice9 *pD3Ddev )
{
	if ( !m_canRender )
	{
		if ( FAILED(CD3DBaseRender::Initialize(pD3Ddev)) )
			return E_FAIL;
		HRESULT vertextinit = m_pD3Ddev->CreateVertexBuffer(m_maxVertex * sizeof(d3dvertex_s),
			D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0, D3DPOOL_DEFAULT, &m_pD3Dbuf, NULL);
		if (FAILED(vertextinit))
		{
			//MessageBoxA(NULL, string_format("Failed init render(%d)[%08x]", vertextinit, vertextinit).c_str(), "Init",0);
			return E_FAIL;
		}
